...
void macro_proccess(char *file_name){
    ...
    FILE *w_file;
    char new_name[strlen(file_name) + 4], *buffer, *macro_name = NULL, *macro_buff, *macro_content = (char* )malloc(LIMIT), *saved_content;
    int i = 0, size = 1, stop = 0, length = 0;
    ENTRY ent, *p_ent;
    strcpy(new_name, file_name);
    strcat(new_name, ".am");
    w_file = fopen(new_name, "w+");
